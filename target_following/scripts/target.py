...
class Target:
    ...
    def __init__(self):
        ...
        mode = input(
            "Enter a mode for the target movement by inputting an interger value, where 1 = hardcoded, 2 = sinusoidal, and 3 = randomized: \n")
    ...
    def main(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.mode == 1:
                ...
                if rospy.Duration(
                        7 * Target.DUR_MUL) <= rospy.get_rostime() - beginning_time and rospy.get_rostime() - beginning_time < rospy.Duration(
                        16 * Target.DUR_MUL):
                    self.straight()
                    self.pub.publish(self.vel_msg)

                if rospy.Duration(
                        16 * Target.DUR_MUL) <= rospy.get_rostime() - beginning_time and rospy.get_rostime() - beginning_time < rospy.Duration(
                        19.75 * Target.DUR_MUL):
                    self.curve_left(0)
                    self.pub.publish(self.vel_msg)

                if rospy.Duration(
                        19.75 * Target.DUR_MUL) <= rospy.get_rostime() - beginning_time and rospy.get_rostime() - beginning_time < rospy.Duration(
                        28.5 * Target.DUR_MUL):
                    self.curve_right(1)
                    self.pub.publish(self.vel_msg)

                if rospy.Duration(
                        28.5 * Target.DUR_MUL) <= rospy.get_rostime() - beginning_time and rospy.get_rostime() - beginning_time < rospy.Duration(
                        41 * Target.DUR_MUL):
                    self.straight()
                    self.pub.publish(self.vel_msg)

                if rospy.Duration(
                        41 * Target.DUR_MUL) <= rospy.get_rostime() - beginning_time and rospy.get_rostime() - beginning_time < rospy.Duration(
                        49 * Target.DUR_MUL):
                    self.curve_right(0)
                    self.pub.publish(self.vel_msg)

                if rospy.Duration(
                        49 * Target.DUR_MUL) <= rospy.get_rostime() - beginning_time and rospy.get_rostime() - beginning_time < rospy.Duration(
                        61 * Target.DUR_MUL):
                    self.straight()
                    self.pub.publish(self.vel_msg)

                if rospy.Duration(
                        61 * Target.DUR_MUL) <= rospy.get_rostime() - beginning_time and rospy.get_rostime() - beginning_time < rospy.Duration(
                        69 * Target.DUR_MUL):
                    self.curve_right(0)
                    self.pub.publish(self.vel_msg)

                if rospy.Duration(
                        69 * Target.DUR_MUL) <= rospy.get_rostime() - beginning_time and rospy.get_rostime() - beginning_time < rospy.Duration(
                        79 * Target.DUR_MUL):
                    self.straight()
                    self.pub.publish(self.vel_msg)

                if rospy.Duration(
                        79 * Target.DUR_MUL) <= rospy.get_rostime() - beginning_time and rospy.get_rostime() - beginning_time < rospy.Duration(
                        89.5 * Target.DUR_MUL):
                    self.curve_left(0)
                    self.pub.publish(self.vel_msg)

                if rospy.Duration(
                        89.5 * Target.DUR_MUL) <= rospy.get_rostime() - beginning_time and rospy.get_rostime() - beginning_time < rospy.Duration(
                        99.5 * Target.DUR_MUL):
                    self.straight()
                    self.pub.publish(self.vel_msg)

                if rospy.Duration(
                        99.5 * Target.DUR_MUL) <= rospy.get_rostime() - beginning_time and rospy.get_rostime() - beginning_time < rospy.Duration(
                        109.25 * Target.DUR_MUL):
                    self.curve_right(0)
                    self.pub.publish(self.vel_msg)

                if rospy.Duration(
                        109.25 * Target.DUR_MUL) <= rospy.get_rostime() - beginning_time and rospy.get_rostime() - beginning_time < rospy.Duration(
                        122.25 * Target.DUR_MUL):
                    self.straight()
                    self.pub.publish(self.vel_msg)

                if rospy.Duration(
                        122.25 * Target.DUR_MUL) <= rospy.get_rostime() - beginning_time and rospy.get_rostime() - beginning_time < rospy.Duration(
                        131.25 * Target.DUR_MUL):
                    self.curve_right(0)
                    self.pub.publish(self.vel_msg)

                if rospy.Duration(
                        131.25 * Target.DUR_MUL) <= rospy.get_rostime() - beginning_time and rospy.get_rostime() - beginning_time < rospy.Duration(
                        140.25 * Target.DUR_MUL):
                    self.straight()
                    self.pub.publish(self.vel_msg)

                if rospy.Duration(
                        140.25 * Target.DUR_MUL) <= rospy.get_rostime() - beginning_time and rospy.get_rostime() - beginning_time < rospy.Duration(
                        146.25 * Target.DUR_MUL):
                    self.curve_right(0)
                    self.pub.publish(self.vel_msg)

                if rospy.Duration(
                        146.25 * Target.DUR_MUL) <= rospy.get_rostime() - beginning_time and rospy.get_rostime() - beginning_time < rospy.Duration(
                        150.25 * Target.DUR_MUL):
                    self.curve_left(0)
                    self.pub.publish(self.vel_msg)

                if rospy.Duration(
                        150.25 * Target.DUR_MUL) <= rospy.get_rostime() - beginning_time and rospy.get_rostime() - beginning_time < rospy.Duration(
                        154.25 * Target.DUR_MUL):
                    self.curve_right(1)
                    self.pub.publish(self.vel_msg)

                if rospy.Duration(
                        154.25 * Target.DUR_MUL) <= rospy.get_rostime() - beginning_time and rospy.get_rostime() - beginning_time < rospy.Duration(
                        155.25 * Target.DUR_MUL):
                    self.straight()
                    self.pub.publish(self.vel_msg)
